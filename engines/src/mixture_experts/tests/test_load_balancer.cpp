...
TEST_F(LoadBalancerTest, CreateLoadBalancerWithValidConfig) {
    ...
    auto load_balancer = lb_result.unwrap();
...
TEST_F(LoadBalancerTest, SelectOptimalExpertBasicSelection) {
    ...
    EXPECT_TRUE(std::find(candidates.begin(), candidates.end(), selected_expert) != candidates.end())
...
TEST_F(LoadBalancerTest, RequestRegistrationAndCompletion) {
    ...
    auto completion_result = load_balancer->register_request_completion(
        expert_id, request_id, 10.0f);
...
TEST_F(LoadBalancerTest, AdaptiveRoutingAdjustments) {
    ...
    for (int i = 0; i < 15; ++i) {
        ...
        load_balancer->register_request_completion(heavy_expert, request_id, 20.0f);  // High latency
    }
...
TEST_F(LoadBalancerTest, ConcurrentLoadTracking) {
    ...
    EXPECT_GE(successful_completions.load(), static_cast<int>(successful_registrations.load() * 0.9f))
...
TEST_F(LoadBalancerTest, ValidateLoadBalancingHealth) {
    ...
    EXPECT_TRUE(health_result.is_ok()) << "Load balancer should remain healthy after normal operation";
}
