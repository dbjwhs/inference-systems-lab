...
protected:
    void SetUp() override {
        ...
        config_.routing_hidden_dimension = 128;
...
TEST_F(ExpertRouterTest, LoadBalancingBehavior) {
    ...
    EXPECT_LT(coefficient_of_variation, 0.5f) 
        << "Expert utilization should be reasonably balanced";
...
TEST_F(ExpertRouterTest, UpdateRoutingParametersWithValidGradients) {
    ...
    std::vector<float> features(256, 1.0f);
    std::vector<float> gradients(config_.routing_hidden_dimension, 0.1f);
    
    auto update_result = router->update_routing_parameters(features, gradients);
