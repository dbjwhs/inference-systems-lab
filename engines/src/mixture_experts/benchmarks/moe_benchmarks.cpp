...
static void SetupMoEEngine() {
    if (!g_test_engine) {
        ...
        if (engine_result.is_ok()) {
            g_test_engine = engine_result.unwrap();
        }
    }
}
...
static void BM_ExpertRouting_Latency(benchmark::State& state) {
    ...
    config.routing_hidden_dimension = 128;
    ...
    auto router = router_result.unwrap();
...
BENCHMARK(BM_MoE_MemoryScaling)
    ->RangeMultiplier(2)
    ->Range(2, 32)
    ->Complexity();
...
static void BM_SparseActivation_Performance(benchmark::State& state) {
    ...
    auto sparse_activation = sparse_result.unwrap();
    ...
    for (auto _ : state) {
        ...
        if (result.is_ok()) {
            ...
            state.counters["elements_per_second"] = benchmark::Counter(
                elements_per_second, benchmark::Counter::kIsRate);
        } else {
...
static void BM_ExpertParameters_LoadingLatency(benchmark::State& state) {
    ...
    auto expert_params = params_result.unwrap();
    ...
    state.counters["loads_per_second"] = benchmark::Counter(
        static_cast<double>(state.iterations()), benchmark::Counter::kIsRate);
}

BENCHMARK(BM_ExpertParameters_LoadingLatency)
    ->UseManualTime()
    ->Iterations(1000);
...
BENCHMARK(BM_MoE_ConfigurationComparison)
    ->DenseRange(0, 2)
    ->UseManualTime();
