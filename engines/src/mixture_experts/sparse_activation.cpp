...
auto SparsePattern::from_dense(const std::vector<float>& dense_data, float threshold) 
    ...
    pattern.sparsity_ratio_ = 1.0f - (static_cast<float>(nnz_count) / static_cast<float>(dense_data.size()));
    return pattern;
}
...
auto SparseActivation::apply_sparse_activation(const std::vector<float>& input,
    ...
    if (expert_weights.empty()) {
        ...
        float weight_factor = expert_weights.empty() ? 1.0f : 
...
auto SparseActivation::adapt_sparsity_threshold(const std::vector<float>& input_distribution) -> float {
...
auto SparseActivation::get_sparse_stats() const -> SparseStats {
    ...
    float total_time = total_computation_time_ms_.load();
...
auto SparseActivation::validate_simd_capabilities() const -> inference_lab::common::Result<std::monostate, MoEError> {
    if (config_.enable_simd_optimization) {
...
auto SparseActivation::benchmark_sparse_performance(std::size_t vector_size, float sparsity_ratio)
    ...
    SparsePattern sparse_pattern = SparsePattern::from_dense(test_vector, 
    ...
    for (int i = 0; i < num_iterations; ++i) {
        auto result = sparse_matrix_vector_multiply(sparse_pattern, weight_matrix, 
