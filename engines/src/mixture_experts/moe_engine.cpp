...
auto MoEEngine::run_inference(const MoEInput& input) 
    ...
    float routing_latency = std::chrono::duration<float, std::milli>(routing_end - routing_start).count();
    ...
    float inference_latency = std::chrono::duration<float, std::milli>(inference_end - inference_start).count();
...
auto MoEEngine::update_performance_metrics(float routing_latency, float inference_latency,
...
auto MoEEngine::validate_system_health() const -> inference_lab::common::Result<std::monostate, MoEError> {
