...
auto LoadBalancer::register_request_completion(std::size_t expert_id,
...

    {
        ...
        float current_avg = expert_load->average_processing_time_ms;
        const float alpha = 0.1f;
        float new_avg = alpha * processing_time_ms + (1.0f - alpha) * current_avg;
        expert_load->average_processing_time_ms = new_avg;
...
auto LoadBalancer::update_expert_load(std::size_t expert_id, float processing_time_ms) -> void {
    ...
    float current_load = expert_loads_[expert_id]->active_requests;
    expert_load_history_[expert_id] =
        alpha * current_load + (1.0f - alpha) * expert_load_history_[expert_id];
    ...
    expert_loads_[expert_id]->current_utilization = std::min(utilization, 1.0f);
    ...
    overall_utilization_variance_ = variance;
}
...
auto LoadBalancer::compute_expert_score(std::size_t expert_id,
    ...
    if (is_expert_overloaded(expert_id)) {
        ...
        float current_utilization = expert_loads_[expert_id]->current_utilization;
...
auto LoadBalancer::select_least_loaded_expert(const std::vector<std::size_t>& candidates)
    ...
    float lowest_utilization = expert_loads_[best_expert]->current_utilization;

    for (std::size_t i = 1; i < candidates.size(); ++i) {
        std::size_t expert_id = candidates[i];
        float utilization = expert_loads_[expert_id]->current_utilization;
...
auto LoadBalancer::apply_load_balancing_penalty(std::size_t expert_id, float base_weight) -> float {
    if (is_expert_overloaded(expert_id)) {
...
auto LoadBalancer::get_load_balancing_stats() const -> LoadBalancingStats {
    ...
    for (std::size_t i = 0; i < config_.num_experts; ++i) {
        ...
        stats.expert_utilization_rates.push_back(expert_load->current_utilization);
        stats.expert_request_counts.push_back(expert_load->active_requests);
        stats.expert_average_latencies.push_back(expert_load->average_processing_time_ms);
    }

    stats.overall_utilization_variance = overall_utilization_variance_;
    stats.load_balance_coefficient = calculate_load_balance_coefficient();
    stats.total_requests_processed = total_requests_processed_;
    stats.total_requests_rejected = total_requests_rejected_;
...
auto LoadBalancer::trigger_expert_rebalancing()
    -> inference_lab::common::Result<std::monostate, MoEError> {
...
auto LoadBalancer::update_routing_adjustments() -> void {
    ...
    float mean_load =
        std::accumulate(expert_load_history_.begin(), expert_load_history_.end(), 0.0f) /
        static_cast<float>(expert_load_history_.size());
...
auto LoadBalancer::handle_expert_failure(std::size_t failed_expert_id)
...

    {
        ...
        expert_loads_[failed_expert_id]->current_utilization = 1.0f;
...
auto LoadBalancer::redistribute_queued_requests(std::size_t failed_expert_id) -> void {
    ...
    if (available_experts.empty()) {
        ...
        total_requests_rejected_ += rejected_count;
...
auto LoadBalancer::validate_load_balancing_health() const
    -> inference_lab::common::Result<std::monostate, MoEError> {
    float variance = overall_utilization_variance_;
