...
ExpertRouter::ExpertRouter(const RouterConfig& config) 
    ...
    expert_selection_counts_.resize(config_.num_experts);
...
auto ExpertRouter::create(const RouterConfig& config) -> Result {
    if (config.num_experts == 0 || config.input_dimension == 0 || 
        config.hidden_dimension == 0 || config.top_k_experts == 0) {
...
auto ExpertRouter::compute_entropy_regularization(const std::vector<float>& probabilities) -> float {
    float entropy = 0.0f;
...
auto ExpertRouter::update_load_balancing(const std::vector<std::size_t>& selected_experts) -> void {
    ...
    for (auto expert_id : selected_experts) {
        if (expert_id < expert_selection_counts_.size()) {
            expert_selection_counts_[expert_id].fetch_add(1);
        }
    }
    ...
    for (std::size_t i = 0; i < config_.num_experts; ++i) {
        expert_load_history_[i] = alpha * current_load[i] + (1.0f - alpha) * expert_load_history_[i];
    }
}
...
auto ExpertRouter::get_routing_stats() const -> RoutingStats {
    ...
    for (std::size_t i = 0; i < config_.num_experts; ++i) {
        std::size_t count = expert_selection_counts_[i].load();
...
auto ExpertRouter::validate_routing_health() const -> VoidResult {
    ...
    if (stats.entropy_score > 10.0f || (stats.total_routing_decisions > 100 && stats.entropy_score < 0.1f)) {
        return inference_lab::common::Err(MoEError::ROUTING_NETWORK_FAILURE);
    }
