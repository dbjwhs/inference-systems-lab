...
auto ExpertParameterHandle::get_parameters()
    ...
    return inference_lab::common::Ok(parameters_);
}
...
auto ExpertParameters::find_cache_entry(std::size_t expert_id)
    -> std::shared_ptr<ExpertParameterHandle> {
...
auto ExpertParameters::get_parameter_stats() const -> ParameterStats {
    ...
    if (stats.total_parameters > 0) {
        stats.compression_efficiency =
            static_cast<float>(compressed_count) / static_cast<float>(stats.total_parameters);
    } else {
...
auto ExpertParameters::calculate_memory_usage() const -> float {
    ...
    total_memory +=
        static_cast<float>(loaded_count * config_.parameters_per_expert * sizeof(float)) /
        (1024.0f * 1024.0f);
    ...
    // Memory for compressed parameters
    {
        ...
        for (const auto& [expert_id, compressed_params] : compressed_parameters_) {
            total_memory +=
                static_cast<float>(compressed_params.size() * sizeof(float)) / (1024.0f * 1024.0f);
        }
    }
    ...
    // Cache memory
    {
        std::lock_guard<std::mutex> lock(cache_mutex_);
        total_memory +=
            static_cast<float>(parameter_cache_.size() * sizeof(CacheEntry)) / (1024.0f * 1024.0f);
    }
...
auto ExpertParameters::cleanup_unused_experts(std::size_t keep_count)
    ...
    std::sort(parameter_cache_.begin(),
              parameter_cache_.end(),
...
auto ExpertParameters::validate_storage_integrity() const
    -> inference_lab::common::Result<std::monostate, MoEError> {
