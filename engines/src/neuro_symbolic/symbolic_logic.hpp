...
public:
    ...
    LogicFormula(LogicOperator quantifier, std::unique_ptr<Variable> var, std::unique_ptr<LogicFormula> body);
    ...
    [[nodiscard]] auto get_operands() const -> const std::vector<std::unique_ptr<LogicFormula>>& { return operands_; }
    ...
    [[nodiscard]] auto apply_substitution(const Substitution& subst) const -> std::unique_ptr<LogicFormula>;
...
public:
     ...
     * @return UnificationResult with substitutions if successful
     */
    static auto unify_predicates(const Predicate& pred1, const Predicate& pred2) -> UnificationResult;
     ...
     */
    static auto apply_substitution(const Term& term, const Substitution& subst) -> std::unique_ptr<Term>;
     ...
     */
    static auto compose_substitutions(const Substitution& subst1, const Substitution& subst2) -> Substitution;

private:
    // Helper methods for unification algorithm
    static auto unify_terms_recursive(const Term& term1, const Term& term2, Substitution& subst) -> bool;
    static auto occurs_check(SymbolId var_id, const Term& term) -> bool;
    static auto apply_substitution_to_substitution(const Substitution& target, const Substitution& subst) -> Substitution;
};
...
public:
     ...
     */
    static auto modus_tollens(const LogicFormula& negated_conclusion, const LogicFormula& implication)
        -> Result<std::unique_ptr<LogicFormula>, LogicError>;
     ...
     */
    static auto existential_generalization(const LogicFormula& formula, const Term& constant, 
...
public:
     ...
     */
    [[nodiscard]] auto answer_query(const LogicFormula& query) const
