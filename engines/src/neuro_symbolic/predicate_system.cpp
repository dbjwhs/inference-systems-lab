...
LogicRule::LogicRule(std::unique_ptr<LogicFormula> head, std::vector<std::unique_ptr<LogicFormula>> body)
...
auto LogicRule::collect_variables() const -> std::unordered_set<SymbolId> {
    ...
    // Collect from head
    auto head_vars = head_->collect_variables();
    variables.insert(head_vars.begin(), head_vars.end());
    
    // Collect from body
    for (const auto& formula : body_) {
        auto body_vars = formula->collect_variables();
...
auto PredicateRegistry::find_signature(const std::string& name, std::size_t arity) const 
    ...
    return std::find_if(signatures_.begin(), signatures_.end(),
        [&name, arity](const PredicateSignature& sig) {
...
SLDReasoner::SLDReasoner(std::shared_ptr<RuleBasedKnowledgeBase> kb) : knowledge_base_(std::move(kb)) {}
...
void SLDReasoner::sld_resolve(const SLDState& state, 
    ...
    if (config.trace_execution) {
        auto trace_entry = "Depth " + std::to_string(state.depth) + ": Resolving " + current_goal.to_string();
...
auto SLDReasoner::apply_substitution_to_goals(const std::vector<std::unique_ptr<LogicFormula>>& goals,
...
auto make_universal(std::unique_ptr<Variable> var, std::unique_ptr<LogicFormula> formula)
    ...
    return std::make_unique<LogicFormula>(LogicOperator::FORALL, std::move(var), std::move(formula));
}
...
auto parse_predicate(const std::string& predicate_str) -> Result<std::unique_ptr<Predicate>, LogicError> {
    ...
    if (!std::regex_match(predicate_str, matches, predicate_regex)) {
        ...
        if (std::regex_match(predicate_str, simple_predicate_regex)) {
            return Ok(std::make_unique<Predicate>(predicate_str, std::vector<std::unique_ptr<Term>>{}));
        }
...
auto parse_rule(const std::string& rule_str) -> Result<std::unique_ptr<LogicRule>, LogicError> {
    ...
    if (rule_separator_pos == std::string::npos) {
        ...
        return Ok(std::make_unique<LogicRule>(std::move(head_formula), std::vector<std::unique_ptr<LogicFormula>>{}));
    }
