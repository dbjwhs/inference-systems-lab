...
LogicFormula::LogicFormula(LogicOperator quantifier, std::unique_ptr<Variable> var, std::unique_ptr<LogicFormula> body)
...
auto LogicFormula::to_string() const -> std::string {
    switch (type_) {
        ...
        case Type::COMPOUND: {
            ...
            if (operands_.size() == 2) {
                return "(" + operands_[0]->to_string() + " " + 
                       neuro_symbolic::to_string(operator_) + " " + 
                       operands_[1]->to_string() + ")";
            }
            ...
            for (std::size_t i = 0; i < operands_.size(); ++i) {
                if (i > 0) oss << " " << neuro_symbolic::to_string(operator_) << " ";
    ...
    switch (type_) {
        ...
        case Type::QUANTIFIED:
            return std::make_unique<LogicFormula>(
                operator_, 
                std::unique_ptr<Variable>(static_cast<Variable*>(quantified_var_->clone().release())),
                operands_[0]->clone()
            );
    }
...
auto LogicFormula::apply_substitution(const Substitution& subst) const -> std::unique_ptr<LogicFormula> {
    switch (type_) {
        case Type::ATOMIC: {
            ...
            auto new_predicate = std::make_unique<Predicate>(predicate_->get_name(), std::move(new_args));
        ...
        case Type::QUANTIFIED: {
            ...
            // For now, just clone without substitution if the quantified variable would be captured
...
auto Unifier::unify_predicates(const Predicate& pred1, const Predicate& pred2) -> UnificationResult {
...
auto Unifier::apply_substitution(const Term& term, const Substitution& subst) -> std::unique_ptr<Term> {
...
auto Unifier::compose_substitutions(const Substitution& subst1, const Substitution& subst2) -> Substitution {
...
auto Unifier::unify_terms_recursive(const Term& term1, const Term& term2, Substitution& subst) -> bool {
    // Same term
    if (&term1 == &term2) {
        return true;
    }
...
auto Unifier::apply_substitution_to_substitution(const Substitution& target, const Substitution& subst) -> Substitution {
    Substitution result;
...
auto InferenceRules::modus_tollens(const LogicFormula& negated_conclusion, const LogicFormula& implication)
    -> Result<std::unique_ptr<LogicFormula>, LogicError> {
...
auto InferenceRules::universal_instantiation(const LogicFormula& universal_formula, const Term& constant)
    -> Result<std::unique_ptr<LogicFormula>, LogicError> {
...
auto InferenceRules::existential_generalization(const LogicFormula& formula, const Term& constant, 
    ...
    return Ok(std::make_unique<LogicFormula>(
        ...
        std::unique_ptr<Variable>(static_cast<Variable*>(variable.clone().release())),
        formula.clone()
    ));
}
...
auto InferenceRules::is_implication(const LogicFormula& formula) -> bool {
    return formula.get_type() == LogicFormula::Type::COMPOUND &&
           formula.get_operator() == LogicOperator::IMPLIES &&
           formula.get_operands().size() == 2;
}
...
auto InferenceRules::formulas_match(const LogicFormula& formula1, const LogicFormula& formula2) -> bool {
