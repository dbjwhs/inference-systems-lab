...
auto main() -> int {
    ...
    auto socrates_fact = FactBuilder("isHuman").withArg("socrates").build();
    ...
    auto age_fact =
        FactBuilder("age").withArg("socrates").withArg(static_cast<int64_t>(70)).build();
    std::cout << "   " << age_fact.toString() << "\n";

    auto lives_in_fact = FactBuilder("livesIn").withArg("socrates").withArg("athens").build();
    ...
    auto wisdom_rule = rule("wisdom_rule")
                           ...
                           .when("age")
                           .withVariable("X")
                           .withArg(static_cast<int64_t>(60))  // age(X, Y) where Y >= 60
                           .then("isWise")
                           .withVariable("X")
                           .withConfidence(0.8)
                           .build();
    ...
    auto find_all_humans = QueryBuilder::findAll().goal("isHuman").withVariable("X").build();
    std::cout << "   " << find_all_humans.toString() << "\n";

    auto prove_mortal = QueryBuilder::prove().goal("isMortal").withArg("socrates").build();
    ...
    std::vector<Value> students = {
        Value::fromText("plato"), Value::fromText("aristotle"), Value::fromText("xenophon")};
    auto teaching_fact = fact("teaches", "socrates", Value::fromList(students));
    std::cout << "   " << teaching_fact.toString() << "\n";

    // Struct value
    std::unordered_map<std::string, Value> person_info = {{"name", Value::fromText("socrates")},
