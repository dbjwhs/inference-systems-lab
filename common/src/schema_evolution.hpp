...
public:
     ...
     */
    SchemaVersion(uint32_t major, uint32_t minor, uint32_t patch, 
...
public:
     ...
     */
    MigrationPath(const SchemaVersion& fromVersion, const SchemaVersion& toVersion,
                  Strategy strategy, bool reversible = false,
...
public:
     ...
     */
    std::vector<std::string> validateEvolution(const schemas::SchemaEvolution::Reader& evolution) const;
...
private:
    ...
    std::unordered_map<std::string, size_t> pathIndex_; // version_string -> index in migrationPaths_
...
public:
     ...
     */
    static std::vector<std::string> generateWarnings(const SchemaVersion& from, const SchemaVersion& to);
};
