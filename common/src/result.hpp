...
class Result {
  public:
    ...
    /**
     * @brief Get the error value (move version), throwing if this Result contains a successful
     * value
    ...
    /**
     * @brief Convert this Result to an optional error (move version), discarding success
     * information
     * @return std::optional containing the error if failed, std::nullopt if successful
     */
     ...
     */
    constexpr bool operator==(const Result& other) const { return data_ == other.data_; }
  ...
  private:
    std::variant<detail::ValueWrapper<T>, detail::ErrorWrapper<E>> data_;  ///< Internal storage
                                                                           ///< using discriminated
                                                                           ///< wrappers
};
...
auto try_call(F&& f) -> Result<std::conditional_t<std::is_void_v<std::invoke_result_t<F>>,
                                                  std::monostate,
...
constexpr auto combine(Results&&... results) {
    ...
    using first_error_type =
        typename std::tuple_element_t<0, std::tuple<std::decay_t<Results>...>>::error_type;
    ...
    if (!(results.is_ok() && ...)) {
        ...
        auto get_first_error = [](auto&&... args) -> Result<value_tuple, first_error_type> {
            ...
            (void)((args.is_err() ? (result = Result<value_tuple, first_error_type>(
                                         Err<first_error_type>(args.unwrap_err())),
                                     true)
                                  : false) ||
                   ...);
            return result;
    ...
    return Result<value_tuple, first_error_type>(
        Ok<value_tuple>(std::make_tuple(std::forward<Results>(results).unwrap()...)));
}
